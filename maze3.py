...
class maze:
    ...
    def unvisited(self, coord):
        row = coord[0]
        col = coord[1]
        if self.matrix[row][col].shape == r"#":
            return True
        else:
            return False
        ...
        for r in range(self.rows):
            for d in dirs:
                if d == "top":
                    ...
                    for c in range(self.cols):
                        if self.matrix[r][c].left and self.matrix[r][c].right:
                            s = s + "|  |"
                        elif self.matrix[r][c].left and not self.matrix[r][c].right:
                            s = s + "|   "
                        elif not self.matrix[r][c].left and self.matrix[r][c].right:
                            s = s + "   |"
                        else:
                            s = s + "    "
                else:
